...
export default function RecycleBinPage() {
  ...
  function handleSelectAll() {
    if (isAllSelected) {
      ...
      const all: Record<string, BinItemSelection> = {};
      novels.forEach((n) => {
        all[`novel:${n.id}`] = { type: 'novel', id: n.id };
      });
      chapters.forEach((c) => {
        all[`chapter:${c.id}`] = { type: 'chapter', id: c.id };
      });
      setSelectedMap(all);
    }
  }
  ...
  async function handleRestoreSingle(type: 'novel' | 'chapter', id: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        if (type === 'novel') setNovels((prev) => prev.filter((n) => n.id !== id));
        if (type === 'chapter') setChapters((prev) => prev.filter((c) => c.id !== id));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="hero-banner p-6 border border-slate-800 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* View Mode Toggle */}
        {!isEmpty && (
          <div className="flex items-center gap-1 bg-slate-950/80 p-1 border border-slate-800 rounded-2xl self-start sm:self-auto">
            <button
      ...
      {!isEmpty && !loading && (
        <div className="p-4 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl flex flex-wrap items-center justify-between gap-3 sticky top-4 z-20 shadow-xl">
          {/* Select All Checkbox Button */}
          <div className="flex items-center gap-3">
            <button
              onClick={handleSelectAll}
              className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-200 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl transition-all"
            >
              {isAllSelected ? (
                  ...
                  {chapters.map((chap) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 bg-slate-900 border rounded-2xl flex flex-col justify-between gap-3 transition-all cursor-pointer ${
                          isSelected
                            ? 'border-amber-500/80 bg-amber-950/15 ring-1 ring-amber-500/30'
                            : 'border-slate-800 hover:border-slate-700'
                        }`}
                      >
                        <div className="flex items-start gap-2.5">
                          {/* Checkbox */}
                          <div
                            className={`p-1 rounded-lg border transition-all flex-shrink-0 mt-0.5 ${
                              isSelected
                                ? 'bg-amber-500 text-slate-950 border-amber-400'
                                : 'bg-slate-950 text-slate-400 border-slate-700'
                            }`}
                        ...
                        <div
                          className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800/80"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button

  ...
  if (!author) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <h2 className="text-lg font-bold text-slate-200">{t('authorNotFound')}</h2>
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl flex items-center gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400">{author.bio || t('unknownAuthor')}</p>
          <div className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md">
            <BookOpen className="w-3 h-3" /> {t('authorWorksCount')}{author.novels?.length || 0}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-base sm:text-lg font-bold text-slate-100">{t('authorAllWorks')}</h2>
